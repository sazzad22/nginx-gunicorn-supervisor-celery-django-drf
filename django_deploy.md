...
## Initial Server Setup:
  ...
  4) Now you can login via ssh like the following and run a command with administrative privileges
...
## Deployment Part I [SSH + Setup Environment]:
 ...
 ### Pull your Django project into server and perform initial Django setup.
   ...
   5) Install the required dependencies of the project.
    
    (venv) $ pip install gunicorn psycopg2-binary wheel
    (venv) $ pip install -r requirements.txt
    
   6) Now create Postgresql database for production server.
   ...
   7) Now you can update your Production Database settings in the following approach:
  ...
  8) Update the `ALLOWED_HOSTS` variable in your `settings.py` file by adding your IP address there.
  ...
  9) Now migrate the initial databse schema to our PostgreSQL database.
  
    (venv) $ python manage.py migrate
    
  10) Run the command to collect all of the static content in to the directory location. You will have to confirm the operation. The static files will then be placed in a directory called static within your project directory. The static files will be saved into the folder which is mentioned in STATIC_ROOT (in the settings.py file)

    (venv) $ python manage.py collectstatic
    
  11) Create an exception for port 8000.
  
    (venv) $ sudo ufw allow 8000
  
  12) Now try to run the server on port 8000 of your server.

    (venv) $ python manage.py runserver 0:8000
    
  13) In your web browser, visit your server’s domain name or IP address followed by :8000:

    (venv) $ http://server_domain_or_IP:8000
    
  14) Let's try to run the project via `Gunicorn`. Try to locate where the `wsgi.py` file is located, and use the following structure: `[foldername of that location].wsgi`
    
    (venv) $ cd ~/testproject
    (venv) $ gunicorn --bind 0.0.0.0:8000 testproject.wsgi

  15) You can repeat step (12) to check running project via gunicorn is working or not.
  16) Now you can deactivate the virtual environment.
